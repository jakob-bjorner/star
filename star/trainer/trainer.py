...
class Trainer:
    ...
    def train_loop(self, progress_bar: tqdm.tqdm, formatted_string_postfix: str):
        ...
        sampled_completitions = wait_for_submitted_jobs(jobs_total, generation_output_dir)
        ...
        train_dataloader = DataLoader(dataset, batch_size=self.train_batch_size, collate_fn=get_collate_fn(tokenizer), num_workers=self.num_workers, pin_memory=True, pin_memory_device="cuda")
        ...
        for data_i, data in enumerate(tqdm.tqdm(fixed_steps_iterator(train_dataloader, local_max_steps), total=local_max_steps)):
            optimizer.zero_grad()
            data.to('cuda')
            if "dpo" in self.train_strategy:
